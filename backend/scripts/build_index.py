...
DATA_FILE = os.path.join(BASE_DIR, "data", "medicine_data.csv")
...
required_columns = ["name", "manufacturer_name", "price", "short_composition1", "short_composition2"]
...
if os.path.exists(pca_file):
    ...
    with open(pca_file, "rb") as f:
        pca = pickle.load(f)
        # Transform in batches to avoid densifying the entire sparse matrix at once
        batch_size = 10000
        num_rows = composition_vectors.shape[0]
        reduced_vectors_batches = []
        for start in tqdm(range(0, num_rows, batch_size), desc="🛠 PCA transform", unit="batch"):
            end = min(start + batch_size, num_rows)
            batch = composition_vectors[start:end].toarray()
            reduced_batch = pca.transform(batch)
            reduced_vectors_batches.append(reduced_batch)
        reduced_vectors = np.vstack(reduced_vectors_batches)
else:
...
# Build or load search index (FAISS if available, else NumPy fallback)
...
if FAISS_AVAILABLE:
    ...
    index = None  # Using NumPy fallback below
    print("ℹ️ FAISS not available. Using NumPy-based nearest neighbor search (may be slower).")

print("🎯 Setup complete! Ready for medicine recommendations.")

def get_correct_name(user_input: str, df: pd.DataFrame, top_n: int = 5, min_similarity: int = 80) -> Optional[str]:
...
def get_generic_alternatives(correct_name: str, df: pd.DataFrame, index, k: int = 3) -> list:
    ...
    if FAISS_AVAILABLE and index is not None:
        ...
        # NumPy fallback: compute L2 distances to all vectors
        diffs = reduced_vectors - query_vector
        dists = np.einsum('ij,ij->i', diffs, diffs)
        # Exclude the same item index
        dists[idx] = np.inf
        alternative_indices = np.argsort(dists)[: (k + 10)]
...
def get_medicine_recommendations(user_input: str) -> list:
    start = time.time()
    ...
    print(f"⏱️ Search time: {time.time() - start:.3f} seconds")
    return alternatives
